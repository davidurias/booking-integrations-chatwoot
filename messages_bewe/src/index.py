...
def process_confirmations(confirmation):
    ...
    start_of_day, end_of_day = common.get_start_end_day(common.convert_date_to_timezone(datetime.now(pytz.utc), confirmation.bewe_client.bewe_account.timezone))

    work_time = common.convert_hour_to_timezone(confirmation.bewe_work.work_time, confirmation.bewe_work.bewe_account.timezone)
    ...
    response = common.message_send(confirmation, content, template_params, session)
...
def process_reminders(reminder):
    start_of_day, end_of_day = common.get_start_end_day(common.convert_date_to_timezone(datetime.now(pytz.utc), reminder.bewe_client.bewe_account.timezone))

    work_time = common.convert_hour_to_timezone(reminder.bewe_work.work_time, reminder.bewe_work.bewe_account.timezone)
...
def handler(event, context):
    ...
    for confirmation in confirmations:
        bewe_client = confirmation.bewe_client  # Assuming `client` is the relationship attribute
